...
//            style={{ background: "linear-gradient(135deg,#1a1200 0%,rgb(var(--color-bg)) 60%)", borderBottom: "1px solid rgb(var(--color-border))" }}>
//         <div className="text-[12px] text-text-muted mb-[2px]">Good morning 👋</div>
...
//         </div>
//         <div className="text-[11px] text-text-muted mt-[6px]">{today}</div>
//       </div>
...
//         {/* Stock Distribution */}
//         <div className="card">
//           <div className="font-syne font-bold text-[14px] text-text mb-3">Stock Distribution</div>
...
//             <div className="font-syne font-bold text-[14px] text-text">Recent Orders</div>
//             <span className="text-[11px] text-accent cursor-pointer">All →</span>
//           </div>

//           {recentOrders.length === 0 ? (
//             <div className="text-[12px] text-text-muted text-center py-4">No orders yet</div>
//           ) : (
...
//       <div className="text-[10px] text-text-muted uppercase tracking-[0.5px] mb-2">{label}</div>
//       <div className="font-syne font-extrabold text-[22px] leading-none text-text">{value}</div>
//       <div className="text-[10px] text-text-muted mt-[5px]">{meta}</div>
//     </div>
//   )
// }
...
//   return (
//     <div className="flex items-center gap-[10px] py-[10px] border-b border-border last:border-0 last:pb-0">
...
//         <div className="text-[11px] text-text-muted mt-[1px] truncate">{partNames}</div>
//       </div>
//       <div className="text-right">
//         <div className="font-syne font-bold text-[13px] text-text">₹{order.total_amount?.toLocaleString() || "—"}</div>
...
export default function Dashboard() {
  ...
  return (
    <div className="flex flex-col h-full bg-bg animate-fadeUp">
      ...
      {/* HERO */}
      <div className="px-5 pt-4 pb-5 flex-shrink-0"
           style={{ background: "var(--dashboard-hero-gradient)", borderBottom: "1px solid rgb(var(--color-border))" }}>
        <div className="text-[12px] text-text-muted mb-[2px]">Good morning 👋</div>
        <div className="font-syne font-extrabold text-[18px] text-text">
          {vendor?.shop_name || "Auto Parts Store"}
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-5 space-y-3">
        ...
        {/* WEEKLY BAR CHART */}
        <div className="card">
          <div className="font-syne font-bold text-[14px] text-text mb-1">Revenue — Last 7 Days</div>
          <div className="text-[11px] text-text-muted mb-3">₹ per day</div>
          {loadingStats ? (
            <div className="h-[130px] flex items-center justify-center text-[12px] text-text-muted">Loading…</div>
          ) : (
            <ResponsiveContainer width="100%" height={130}>
              <BarChart data={chartData} margin={{ top: 0, right: 0, left: -30, bottom: 0 }} barSize={18}>
                ...
                <YAxis tick={{ fill: "rgb(var(--color-text-muted))", fontSize: 9 }} axisLine={false} tickLine={false}
                       tickFormatter={v => v === 0 ? "" : `₹${v}`} />
                <Tooltip
                  cursor={{ fill: "rgba(244,166,35,0.06)" }}
                  contentStyle={{ background: "rgb(var(--color-surface))", border: "1px solid rgb(var(--color-border))", borderRadius: 10, fontSize: 11 }}
                  labelStyle={{ color: "rgb(var(--color-text-primary))" }}
                  formatter={v => [`₹${v.toLocaleString("en-IN")}`, "Revenue"]}
                />
                <Bar dataKey="revenue" radius={[6, 6, 0, 0]}>
                  {chartData.map((entry, i) => (
                    <Cell key={i} fill={entry.revenue === maxRev && entry.revenue > 0 ? "#f4a623" : "rgb(var(--color-border))"} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        {/* ORDER SUMMARY */}
        <div className="card">
          <div className="font-syne font-bold text-[14px] text-text mb-3">Order Summary</div>
...
function StatTile({ label, value, meta, color, icon }) {
  ...
  return (
    <div className="flex-shrink-0 min-w-[130px] bg-surface rounded-2xl p-[14px] relative overflow-hidden"
      ...
      <div className="absolute bottom-2 right-3 text-[26px] opacity-[0.07]">{icon}</div>
      <div className="text-[10px] text-text-muted uppercase tracking-[0.5px] mb-2">{label}</div>
      <div className="font-syne font-extrabold text-[20px] leading-none text-text">{value}</div>
      <div className="text-[10px] text-text-muted mt-[5px]">{meta}</div>
    </div>
  )
}
...
function SummaryRow({ label, value, color }) {
  return (
    <div className="flex justify-between items-center py-[8px] border-b border-border last:border-0">
