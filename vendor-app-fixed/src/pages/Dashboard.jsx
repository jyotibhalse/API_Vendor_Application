...
import { AlertTriangle, TrendingUp, Package, ShoppingCart, AlertCircle } from "lucide-react"
import {
  ...
  return (
    <div className="flex flex-col h-full bg-bg animate-fadeUp">
      ...
      <div
        ...
        style={{
          background: "linear-gradient(135deg,#1a1200 0%,#0c0d0f 60%)",
          borderBottom: "1px solid #252830",
        }}
      >
        <div className="text-[12px] text-[#9ca3af] mb-[2px]">{getGreeting()}</div>
      ...
      <div className="flex gap-[10px] px-5 pb-2 overflow-x-auto flex-shrink-0">
        <StatTile
          label="Revenue"
          value={loadingStats ? "…" : `₹${(stats?.revenue || 0).toLocaleString("en-IN")}`}
          meta={activeFilter}
          color="amber"
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-5 space-y-3">
        ...
        {/* Revenue bar chart */}
        <div className="card">
          <div className="font-syne font-bold text-[14px] text-white mb-[2px]">Revenue — Last 7 Days</div>
          <div className="text-[11px] text-[#9ca3af] mb-3">Always shows the past 7 days</div>
          {loadingStats ? (
            <div className="h-[130px] flex items-center justify-center text-[12px] text-[#9ca3af]">Loading…</div>
          ) : chartData.every(d => d.revenue === 0) ? (
            <div className="h-[130px] flex items-center justify-center text-[12px] text-[#9ca3af]">
              ...
              <BarChart data={chartData} margin={{ top: 0, right: 0, left: -30, bottom: 0 }} barSize={18}>
                ...
                <Bar dataKey="revenue" radius={[6, 6, 0, 0]}>
                  {chartData.map((entry, i) => (
                    <Cell
                      key={i}
                      fill={entry.revenue === maxRev && entry.revenue > 0 ? "#f4a623" : "#252830"}
                    />
                  ))}
                </Bar>
              </BarChart>
        ...
        {/* Order Summary */}
        <div className="card">
          <div className="font-syne font-bold text-[14px] text-white mb-3">
            Order Summary
            <span className="ml-2 text-[11px] font-normal text-[#9ca3af]">— {activeFilter}</span>
          </div>
          ...
          <SummaryRow
            label="Rejected"
            value={(stats?.order_count ?? 0) - (stats?.pending_count ?? 0) - (stats?.completed_count ?? 0)}
            color="#ef4444"
          />
        </div>
        ...
        <div className="card">
          ...
          {loadingStats ? (
            <div className="text-[12px] text-[#9ca3af] text-center py-4">Loading…</div>
...
function StatTile({ label, value, meta, color, icon }) {
  ...
  return (
    <div
      className="flex-shrink-0 min-w-[130px] bg-surface rounded-2xl p-[14px] relative overflow-hidden"
...
function OrderRow({ order, onPress }) {
  ...
  return (
    <button
      onClick={onPress}
      className="w-full flex items-center gap-[10px] py-[10px] border-b border-[#252830] last:border-0 last:pb-0 text-left"
    >
      <div
      ...
      <div className="text-right flex-shrink-0">
        <div className="font-syne font-bold text-[13px] text-white">
