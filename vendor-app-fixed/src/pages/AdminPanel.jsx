...
function HeroMeta({ icon: Icon, label, value }) {
  return (
    ...
    >
      ...
      <div className="mt-2 break-all text-[13px] font-semibold leading-tight text-text sm:text-[14px]">{value}</div>
    </div>
  )
}
...
function StatTile({ icon: Icon, label, value, meta, tone = "amber" }) {
  ...
  return (
    ...
    >
      <div className="absolute left-0 right-0 top-0 h-[3px]" style={{ background: palette.bar }} />
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-[10px] uppercase tracking-[0.5px] text-text-muted">{label}</div>
          <div className="mt-3 font-syne text-[22px] font-extrabold leading-none text-text">{value}</div>
...
export default function AdminPanel() {
    ...
    try {
      ...
      setSettingsForm({
        default_commission_rate:
          overviewData.settings?.default_commission_rate ?? INITIAL_SETTINGS.default_commission_rate,
        platform_fee_flat:
          overviewData.settings?.platform_fee_flat ?? INITIAL_SETTINGS.platform_fee_flat,
      })
      setCommissionDrafts((current) => {
        const nextDrafts = {}
        vendorItems.forEach((vendorItem) => {
          nextDrafts[vendorItem.id] =
            vendorItem.commission_rate == null ? "" : String(vendorItem.commission_rate)
        })
      ...
      setNotesDrafts((current) => {
        const nextDrafts = {}
        vendorItems.forEach((vendorItem) => {
          nextDrafts[vendorItem.id] = current[vendorItem.id] ?? vendorItem.approval_notes ?? ""
        })
        return nextDrafts
      })
    } catch (err) {
      setError(err.response?.data?.detail || "Unable to load admin analytics right now.")
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const handleApproval = async (vendorId, status) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || `Unable to ${status} this vendor right now.`)
    } finally {
      setBusyKey("")
    }
  }
  ...
  const handleCommissionSave = async (vendorId) => {
    ...
    try {
      await api.patch(`/admin/vendors/${vendorId}/commission`, {
        commission_rate: commissionDrafts[vendorId] === "" ? null : Number(commissionDrafts[vendorId]),
      })
      setNotice("Commission override saved.")
      await loadAdminData(activeFilter)
    } catch (err) {
      setError(err.response?.data?.detail || "Unable to save commission override.")
    } finally {
      setBusyKey("")
    }
  }
  ...
  const handleSettingsSubmit = async (event) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || "Unable to update platform settings.")
    } finally {
  ...
  const topVendors = (overview?.top_vendors || []).filter((vendorItem) => vendorItem.approval_status !== "rejected")
  const pendingVendors = filteredVendors.filter((vendorItem) => vendorItem.approval_status === "pending")

  return (
    <div className="flex h-full min-h-screen flex-col overflow-hidden bg-bg animate-fadeUp">
      <AppTopHeader homeTo="/admin" actions={[{ icon: LogOut, label: "Logout", onClick: handleLogout }]} />
        ...
        <div className="w-full space-y-4 px-4 pt-4 pb-6 sm:space-y-5 sm:px-5 sm:pt-5 sm:pb-8 lg:mx-auto lg:max-w-6xl">
          ...
          >
            ...
            <div className="mt-4 grid gap-2.5 sm:grid-cols-2 sm:gap-3">
              <HeroMeta icon={Users} label="Pending Vendors" value={loading ? "Loading..." : summary.pending_vendors ?? 0} />
              <HeroMeta icon={Wallet} label="Platform Earnings" value={loading ? "Loading..." : formatCurrency(summary.platform_earnings)} />
              <HeroMeta icon={Receipt} label="Gross Revenue" value={loading ? "Loading..." : formatCurrency(summary.gross_revenue)} />
              <HeroMeta icon={TrendingUp} label="Default Commission" value={formatPercent(settingsForm.default_commission_rate)} />
            </div>
            ...
            <div className="mt-4">
              ...
              >
                <div className="text-[10px] uppercase tracking-[0.6px] text-text-muted">Platform Fee</div>
          ...
          <div className="grid gap-5">
            <SectionCard>
              ...
              <div className="mt-2 font-syne text-[20px] font-bold text-text">Revenue vs platform earnings</div>
              <div className="mt-1 text-[12px] text-text-muted">Daily movement across the current time filter.</div>
              ...
              <div className="mt-4 h-[220px] sm:mt-5 sm:h-[280px]">
                {loading ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chart} margin={{ top: 8, right: 0, left: -34, bottom: 0 }}>
                      <defs>
                        <linearGradient id="adminRevenueFill" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="5%" stopColor="#f4a623" stopOpacity={0.28} />
                          <stop offset="95%" stopColor="#f4a623" stopOpacity={0.02} />
                        </linearGradient>
                        <linearGradient id="adminPlatformFill" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="5%" stopColor="#3b82f6" stopOpacity={0.22} />
                          <stop offset="95%" stopColor="#3b82f6" stopOpacity={0.02} />
                        </linearGradient>
                      </defs>
                      <CartesianGrid stroke="rgba(255,255,255,0.06)" strokeDasharray="4 4" vertical={false} />
                      <XAxis dataKey="day" tick={{ fill: "rgb(var(--color-text-muted))", fontSize: 10 }} axisLine={false} tickLine={false} />
                      <YAxis
                      ...
                      />
                      <Area type="monotone" dataKey="revenue" stroke="#f4a623" strokeWidth={2.5} fill="url(#adminRevenueFill)" />
                      <Area type="monotone" dataKey="platform_earnings" stroke="#3b82f6" strokeWidth={2} fill="url(#adminPlatformFill)" />
                    </AreaChart>
                  </ResponsiveContainer>
                )}
              </div>
              ...
              <SectionCard>
                ...
                <div className="mt-2 font-syne text-[20px] font-bold text-text">Platform fee settings</div>
              ...
              <SectionCard>
                ...
                <div className="mt-2 font-syne text-[20px] font-bold text-text">Best performers this period</div>
                <div className="mt-1 text-[12px] text-text-muted">Quick revenue leaderboard for admin review.</div>
                        ...
                        <div className="flex items-center justify-between gap-3">
                          <div className="min-w-0">
                            <div className="truncate font-syne text-[15px] font-bold text-text">
                              {vendorItem.shop_name || vendorItem.full_name || vendorItem.email}
                            </div>
                            <div className="mt-1 text-[11px] text-text-muted">
                              {vendorItem.order_count} orders • {formatPercent(vendorItem.effective_commission_rate)} commission
                            </div>
                          </div>
          ...
          <SectionCard>
            <div className="min-w-0">
                ...
                <div className="mt-2 font-syne text-[20px] font-bold text-text">Vendors awaiting review</div>
                <div className="mt-1 text-[12px] text-text-muted">Approve good fits quickly or reject with review notes.</div>
                <div
                ...
                >
                  <span className="break-words">{pendingVendors.length} pending in the current filter</span>
                </div>
            </div>
            ...
            <div className="mt-4 grid gap-4">
              {pendingVendors.length === 0 ? (
                <div
                  ...
                  style={{ background: "rgb(var(--color-surface-2))", border: "1px dashed rgb(var(--color-border))" }}
                >
                ...
                pendingVendors.map((vendorItem) => (
                  ...
                  >
                    ...
                    <div className="mt-4 grid gap-3 sm:grid-cols-3">
                      <MetricPill label="Revenue" value={formatCurrency(vendorItem.gross_revenue)} />
                      <MetricPill label="Orders" value={vendorItem.order_count} />
                      <MetricPill label="Commission" value={formatPercent(vendorItem.effective_commission_rate)} />
                    </div>
                    ...
                    <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                      <button
                        ...
                        disabled={busyKey === `approval-${vendorItem.id}-approved`}
                        ...
                        style={{ opacity: busyKey === `approval-${vendorItem.id}-approved` ? 0.75 : 1 }}
                      >
                        <CheckCircle2 size={15} />
                        {busyKey === `approval-${vendorItem.id}-approved` ? "Approving..." : "Approve Vendor"}
                      </button>

                      <button
                        type="button"
                        onClick={() => handleApproval(vendorItem.id, "rejected")}
                        disabled={busyKey === `approval-${vendorItem.id}-rejected`}
                        ...
                        style={{
                          ...
                          border: "1px solid var(--feedback-error-border)",
                          color: "var(--feedback-error-text)",
                          opacity: busyKey === `approval-${vendorItem.id}-rejected` ? 0.75 : 1,
                        }}
                      >
                        <XCircle size={15} />
                        {busyKey === `approval-${vendorItem.id}-rejected` ? "Rejecting..." : "Reject Vendor"}
                      </button>
                    </div>
                  </div>
                ))
                  ...
                  >
                    <div className="min-w-0">
                      <div className="min-w-0">
                        ...
                        <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 break-words text-[12px] text-text-muted">
                          {vendorItem.full_name || "Vendor account"} • {vendorItem.email}
                        </div>
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-3">
                          <MetricPill label="Gross Revenue" value={formatCurrency(vendorItem.gross_revenue)} />
                          <MetricPill label="Platform Earnings" value={formatCurrency(vendorItem.platform_earnings)} />
                          <MetricPill label="Net Vendor Revenue" value={formatCurrency(vendorItem.net_revenue)} />
                          <MetricPill label="Total Orders" value={vendorItem.order_count} />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-4 space-y-4">
                      <div className="grid grid-cols-2 gap-3">
                        <MetricPill label="Pending" value={vendorItem.pending_orders} />
                        <MetricPill label="Delivered" value={vendorItem.delivered_orders} />
                        <MetricPill label="Rejected" value={vendorItem.rejected_orders} />
                        <MetricPill label="Effective Commission" value={formatPercent(vendorItem.effective_commission_rate)} />
                      </div>
                      ...
                      <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end xl:max-w-[440px]">
                        ...
                        <button
                          ...
                          style={{ opacity: busyKey === `commission-${vendorItem.id}` ? 0.75 : 1 }}
                        >
                          {busyKey === `commission-${vendorItem.id}` ? "Saving..." : "Save Override"}
                        </button>
                      </div>
                    </div>
